...
async def test_kiwi_connection():
    """测试 KiwiSDR 连接"""
    print("=== KiwiSDR 连接测试 ===")
    
    # 一些公共 KiwiSDR 服务器示例
    test_servers = [
        {"name": "示例服务器1", "host": "sdr.example.com", "port": 8073},
        {"name": "示例服务器2", "host": "kiwisdr.example.org", "port": 8073},
    ]
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(http_url, timeout=10) as response:
                    if response.status == 200:
                        ...
                        print(f"⚠ {server['name']} 服务器返回状态: {response.status}")
...
async def main():
    ...
    if connection_ok:
        ...
        print("命令: python -m echofist monitor --server kiwisdr.pa3gjx.nl:8073 --freq 7.023")
        return 0
    else:
        print("\n⚠ 连接测试遇到问题")
        print("请检查:")
